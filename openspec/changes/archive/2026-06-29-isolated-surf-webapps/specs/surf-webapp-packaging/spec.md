...
Webapps SHALL execute `surf` with strictly scoped and overriden `XDG_DATA_HOME`, `XDG_CACHE_HOME`, and `XDG_CONFIG_HOME` pointing to `~/.config/surf-app-profiles/<app_id>`.

#### Scenario: Launching an isolated webapp
- **WHEN** the user executes the webapp wrapper
- **THEN** it sets `XDG_DATA_HOME`, `XDG_CACHE_HOME`, and `XDG_CONFIG_HOME` for the `surf` process exclusively to the isolated profile path.
...
When intercepting a link to an external browser, the wrapper SHALL restore the original `XDG_` environment variables of the user.

#### Scenario: Launching external browser
- **WHEN** the wrapper calls `xdg-open` to handle an intercepted URL
- **THEN** the environment passed to `xdg-open` must contain the original `XDG_CONFIG_HOME`, `XDG_CACHE_HOME`, and `XDG_DATA_HOME` values that the user had before launching the webapp, or unset them if they were not originally set.
